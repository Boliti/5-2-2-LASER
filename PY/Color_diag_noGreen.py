...
if os.path.exists(file_path):
    data_nogreen= all_data[all_data['COLOR'] != 'Green']
    # Подсчет количества записей по цветам
    color_counts = data_nogreen['COLOR'].value_counts() 
    total_count = color_counts.sum()
    filtered_color_counts = color_counts[color_counts / total_count >= 0.01]
